...
// NewGitPR creates a new GitPRTool instance.
func NewGitPR(workDir string) *GitPRTool {
...
func (t *GitPRTool) Execute(ctx context.Context, args map[string]any) (*sdk.ToolResult, error) {
	if _, err := exec.LookPath("gh"); err != nil {
		return sdk.NewErrorResult("gh CLI is not installed. Install it from https://cli.github.com/"), nil
	}
...
func (t *GitPRTool) createPR(ctx context.Context, args map[string]any) (*sdk.ToolResult, error) {
	title, _ := sdk.GetString(args, "title")
	body, _ := sdk.GetString(args, "body")
	base := sdk.GetStringDefault(args, "base", "")
	draft := sdk.GetBoolDefault(args, "draft", false)
	autoDesc := sdk.GetBoolDefault(args, "auto_description", false)
	...
	if title == "" {
		return sdk.NewErrorResult("title is required (or use auto_description=true)"), nil
	}
	...
	if err != nil {
		...
		if !strings.Contains(outStr, "Everything up-to-date") {
			return sdk.NewErrorResult(fmt.Sprintf("failed to push branch: %s\n%s", err, outStr)), nil
		}
	}
	...
	if err != nil {
		return sdk.NewErrorResult(fmt.Sprintf("failed to create PR: %s\n%s", err, string(output))), nil
	}

	return sdk.NewSuccessResult(fmt.Sprintf("Pull request created:\n%s", strings.TrimSpace(string(output)))), nil
}

func (t *GitPRTool) listPRs(ctx context.Context) (*sdk.ToolResult, error) {
	cmd := exec.CommandContext(ctx, "gh", "pr", "list", "--limit", "20")
	cmd.Dir = t.workDir
	output, err := cmd.CombinedOutput()
	if err != nil {
		return sdk.NewErrorResult(fmt.Sprintf("failed to list PRs: %s\n%s", err, string(output))), nil
	}

	result := strings.TrimSpace(string(output))
	if result == "" {
		return sdk.NewSuccessResult("No open pull requests."), nil
	}
	return sdk.NewSuccessResult(result), nil
}

func (t *GitPRTool) viewPR(ctx context.Context, args map[string]any) (*sdk.ToolResult, error) {
	prNum, _ := sdk.GetString(args, "pr_number")
	if prNum == "" {
		return sdk.NewErrorResult("pr_number is required for view"), nil
	}
	...
	output, err := cmd.CombinedOutput()
	if err != nil {
		return sdk.NewErrorResult(fmt.Sprintf("failed to view PR: %s\n%s", err, string(output))), nil
	}
	return sdk.NewSuccessResult(strings.TrimSpace(string(output))), nil
}

func (t *GitPRTool) checksPR(ctx context.Context, args map[string]any) (*sdk.ToolResult, error) {
	prNum, _ := sdk.GetString(args, "pr_number")
	if prNum == "" {
		return sdk.NewErrorResult("pr_number is required for checks"), nil
	}

	cmd := exec.CommandContext(ctx, "gh", "pr", "checks", prNum)
	cmd.Dir = t.workDir
	output, _ := cmd.CombinedOutput()
	return sdk.NewSuccessResult(fmt.Sprintf("PR #%s checks:\n%s", prNum, strings.TrimSpace(string(output)))), nil
}

func (t *GitPRTool) mergePR(ctx context.Context, args map[string]any) (*sdk.ToolResult, error) {
	prNum, _ := sdk.GetString(args, "pr_number")
	if prNum == "" {
		return sdk.NewErrorResult("pr_number is required for merge"), nil
	}
	...
	output, err := cmd.CombinedOutput()
	if err != nil {
		return sdk.NewErrorResult(fmt.Sprintf("failed to merge PR: %s\n%s", err, string(output))), nil
	}
	return sdk.NewSuccessResult(fmt.Sprintf("PR #%s merged.\n%s", prNum, strings.TrimSpace(string(output)))), nil
}

func (t *GitPRTool) closePR(ctx context.Context, args map[string]any) (*sdk.ToolResult, error) {
	prNum, _ := sdk.GetString(args, "pr_number")
	if prNum == "" {
		return sdk.NewErrorResult("pr_number is required for close"), nil
	}
	...
	output, err := cmd.CombinedOutput()
	if err != nil {
		return sdk.NewErrorResult(fmt.Sprintf("failed to close PR: %s\n%s", err, string(output))), nil
	}
	return sdk.NewSuccessResult(fmt.Sprintf("PR #%s closed.\n%s", prNum, strings.TrimSpace(string(output)))), nil
}
...
func (t *GitPRTool) generatePRDescription(ctx context.Context, base string) (string, string) {
	...
	if len(statOutput) > 0 {
		...
		bodyBuilder.WriteString(strings.TrimSpace(string(statOutput)))
		bodyBuilder.WriteString("\n```\n")
	}

	bodyBuilder.WriteString("\n## Test plan\n\n- [ ] Tests pass\n- [ ] Manual verification\n")

	return title, bodyBuilder.String()
}
