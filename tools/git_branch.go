...
// NewGitBranch creates a new GitBranchTool instance.
func NewGitBranch(workDir string) *GitBranchTool {
...
func (t *GitBranchTool) Execute(ctx context.Context, args map[string]any) (*sdk.ToolResult, error) {
...
func (t *GitBranchTool) listBranches(ctx context.Context, args map[string]any) (*sdk.ToolResult, error) {
	cmdArgs := []string{"branch", "--format=%(refname:short) %(objectname:short) %(subject)"}
	if sdk.GetBoolDefault(args, "all", false) {
	...
	if err != nil {
		return sdk.NewErrorResult(fmt.Sprintf("git branch list failed: %s\n%s", err, string(output))), nil
	}
	...
	return sdk.NewSuccessResult(result.String()), nil
}

func (t *GitBranchTool) createBranch(ctx context.Context, args map[string]any) (*sdk.ToolResult, error) {
	name, _ := sdk.GetString(args, "name")
	if name == "" {
		return sdk.NewErrorResult("name is required for create"), nil
	}

	from, _ := sdk.GetString(args, "from")
	...
	output, err := cmd.CombinedOutput()
	if err != nil {
		return sdk.NewErrorResult(fmt.Sprintf("failed to create branch: %s\n%s", err, string(output))), nil
	}

	return sdk.NewSuccessResult(fmt.Sprintf("Created and switched to branch '%s'", name)), nil
}

func (t *GitBranchTool) deleteBranch(ctx context.Context, args map[string]any) (*sdk.ToolResult, error) {
	name, _ := sdk.GetString(args, "name")
	if name == "" {
		return sdk.NewErrorResult("name is required for delete"), nil
	}

	force := sdk.GetBoolDefault(args, "force", false)
	flag := "-d"
	if force {
		flag = "-D"
	}
	...
	if err != nil {
		...
		if strings.Contains(outStr, "not fully merged") {
			return sdk.NewErrorResult(fmt.Sprintf("branch '%s' is not fully merged. Use force=true to delete anyway.", name)), nil
		}
		return sdk.NewErrorResult(fmt.Sprintf("failed to delete branch: %s\n%s", err, outStr)), nil
	}

	return sdk.NewSuccessResult(fmt.Sprintf("Deleted branch '%s'", name)), nil
}

func (t *GitBranchTool) switchBranch(ctx context.Context, args map[string]any) (*sdk.ToolResult, error) {
	name, _ := sdk.GetString(args, "name")
	if name == "" {
		return sdk.NewErrorResult("name is required for switch"), nil
	}
	...
	if err != nil {
		...
		if strings.Contains(outStr, "local changes") {
			return sdk.NewErrorResult(fmt.Sprintf("cannot switch to '%s': you have local changes. Commit or stash them first, or use force=true.", name)), nil
		}
		return sdk.NewErrorResult(fmt.Sprintf("failed to switch branch: %s\n%s", err, outStr)), nil
	}

	return sdk.NewSuccessResult(fmt.Sprintf("Switched to branch '%s'", name)), nil
}

func (t *GitBranchTool) mergeBranch(ctx context.Context, args map[string]any) (*sdk.ToolResult, error) {
	name, _ := sdk.GetString(args, "name")
	if name == "" {
		return sdk.NewErrorResult("name is required for merge"), nil
	}
	...
	if err != nil {
		if strings.Contains(outStr, "CONFLICT") {
			...
			return sdk.NewErrorResult(fmt.Sprintf("Merge conflict when merging '%s'.\n\nConflicting files:\n%s\nResolve conflicts and commit, or run 'git merge --abort' to cancel.",
				name, strings.TrimSpace(string(conflictOutput)))), nil
		}
		return sdk.NewErrorResult(fmt.Sprintf("merge failed: %s\n%s", err, outStr)), nil
	}

	return sdk.NewSuccessResult(fmt.Sprintf("Merged '%s' into current branch.\n%s", name, outStr)), nil
}

func (t *GitBranchTool) currentBranch(ctx context.Context) (*sdk.ToolResult, error) {
	...
	if err != nil {
		...
		if hashErr != nil {
			return sdk.NewErrorResult("not in a git repository"), nil
		}
		return sdk.NewSuccessResult(fmt.Sprintf("HEAD detached at %s", strings.TrimSpace(string(hashOutput)))), nil
	}
