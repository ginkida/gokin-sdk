...
func (c *Checker) GetLatestRelease(ctx context.Context) (*ReleaseInfo, error) {
	...
	release, err := c.fetchRelease(ctx, u)
	if err != nil {
		return nil, err
	}
...
func (c *Checker) GetReleases(ctx context.Context, limit int) ([]ReleaseInfo, error) {
	u := fmt.Sprintf("https://api.github.com/repos/%s/releases?per_page=%d", c.repo, limit)
...
func (c *Checker) GetReleaseByTag(ctx context.Context, tag string) (*ReleaseInfo, error) {
	u := fmt.Sprintf("https://api.github.com/repos/%s/releases/tags/%s", c.repo, tag)
	return c.fetchRelease(ctx, u)
}

// fetchRelease performs the HTTP request to fetch a release.
func (c *Checker) fetchRelease(ctx context.Context, u string) (*ReleaseInfo, error) {
	req, err := http.NewRequestWithContext(ctx, http.MethodGet, u, nil)
	if err != nil {
		return nil, err
	}
